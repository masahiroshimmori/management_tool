...
function login($input_pass, $datum, $lock_table) {
    ...
    // admin_userが存在していたら、作業を続行する
    if (false === empty($datum)) {
        // ロックテーブルを読み込んで情報を把握する
        $sql = 'SELECT * FROM ' . $lock_table . ' WHERE admin_id=:admin_id;';
        $pre = $dbh->prepare($sql);
        // 値のバインド
        $pre->bindValue(':admin_id', $datum['admin_id'], PDO::PARAM_STR);
        // SQLの実行
        $r = $pre->execute(); // XXX
        // SELECTした内容の取得
        ...
        if (false === $lock_datum) {
            //
            $lock_datum['admin_id'] = $datum['admin_id'];
        ...
        // ロックテーブルに情報を入れる
        // 
        $sql = 'REPLACE INTO ' . $lock_table . '(admin_id, error_count, lock_time) VALUES(:admin_id, :error_count, :lock_time);';
        $pre = $dbh->prepare($sql);
        // 値のバインド
        $pre->bindValue(':admin_id', $lock_datum['admin_id'], PDO::PARAM_STR);
